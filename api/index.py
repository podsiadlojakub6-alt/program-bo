...
def index():
    ...
    if request.method == 'POST':
        try:
            # Pobranie i walidacja współczynnika gamma
            gamma_val = request.form.get('gamma', '0.6').replace(',', '.')
            gamma = float(gamma_val) if gamma_val else 0.6
            ...
            # Jeśli macierz jest pusta, przerywamy
            if max_row == -1:
                return render_template('index.html', results=None)

            # Budowa macierzy o wymiarach (max_row+1) x (max_col+1)
            ...
            # 3. Bayes (Laplace - równe prawdopodobieństwa)
            ...
            # Przygotowanie wyników w formacie czytelnym dla HTML
            results = {
